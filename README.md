...
2.  Suppose you use a binary search tree to store the words from the 
    ...
    > In the worst case scenario for unbalanced binary search tree is O(n) = 127142 
...
3.  Suppose you use a prefix tree to store the words from the `dictionary.txt` 
    ...
    > The worse case scenario for a prefix tree would be the length of the word (key).

4.  Why is the prefix tree better than a binary search tree for implementing 
    Boggle?

    > The prefix tree is better because you do not need to look through half the binary tree
>     you only need to look for the word with the specific prefix.

5.  Suppose you do not use `IsPrefix` in your `SolveBoard` implementation.  
    How would that affect the program?

    > The way that it would affect solveboard is that the program would take longer to execute because
>     you would need to look through the dictionary and search for the prefix.
